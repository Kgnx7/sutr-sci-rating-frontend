...
import { getFacultyDepartments } from "../../app/departmentsSlice";
import {
  VirtualTableState,
  SortingState,
  SearchState,
...
function FacultyDepartments({ id }) {
  ...
  const getDepartments = () => {
    dispatch(getFacultyDepartments(id, history));
  };
...
export default function FacultyShow() {
  ...
  const { id } = useParams();
  const faculty = faculties.find((faculty) => faculty.id === parseInt(id));
          ...
          <>
            ...
            <Box mt={3}>
              ...
              <TabPanel value={tab} index={1}>
                <FacultyDepartments id={faculty.id} />
              </TabPanel>
            </Box>
          </>
        )}
